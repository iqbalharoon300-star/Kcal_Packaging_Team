...
def overtime_from_total(total_hours, regular_hours=8.0):
...
def dashboard():
    # stats
    total_employees = User.query.filter(User.role=="team").count()
    ...
    total_ot = db.session.query(db.func.sum(OvertimeRecord.overtime_hours)).filter(OvertimeRecord.date>=first_day, OvertimeRecord.date<=last_day).scalar() or 0
    pending = OvertimeRecord.query.filter(OvertimeRecord.sign_of_incharge=="").count()
    return render_template("dashboard.html", total_employees=total_employees, total_ot=round(total_ot,2), pending=pending, config=app.config)
...
def download_excel():
    if current_user.role not in ["manager","supervisor"]:
        flash("Only Manager or Supervisor can download Excel", "danger")
        return redirect(url_for("dashboard"))
    ...
    return send_file(bio, attachment_filename=filename, as_attachment=True, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
